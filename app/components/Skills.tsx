import { SiNextdotjs, SiTypescript, SiReact, SiTailwindcss, SiGit, SiGithub } from 'react-icons/si'
...
const skills: { name: string; Icon: IconType; className: string; iconClass: string }[] = [
  ...
  { name: 'Git', Icon: SiGit, className: 'border-[#f05032]/40 bg-[#f05032]/10 hover:border-[#f05032]', iconClass: 'text-[#f05032]' },
  { name: 'GitHub', Icon: SiGithub, className: 'border-white/20 bg-white/5 hover:border-white/50', iconClass: 'text-white' },
]

export function Skills() {
  return <section id="habilidades" className="border-t border-white/10 px-5 py-24 lg:px-8"><div className="mx-auto max-w-6xl"><p className="font-mono text-sm uppercase tracking-[.25em] text-cyan">Stack principal</p><h2 className="mt-3 font-display text-3xl font-semibold text-white sm:text-5xl">Herramientas para <span className="gradient-text">crear mejor.</span></h2><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{skills.map(({ name, Icon, className, iconClass }, index) => <div key={name} className={`glass neon-border animate-rise rounded-2xl p-7 transition duration-500 hover:-translate-y-3 hover:shadow-[0_0_32px_rgba(0,212,255,.15)] ${className}`} style={{ animationDelay: `${index * 80}ms` }}><Icon className={`mb-7 text-5xl ${iconClass}`} aria-hidden="true" /><h3 className="font-display text-lg font-semibold text-white">{name}</h3></div>)}</div></div></section>
}
